<?php
/**
 * RelatorioList Listing
 * @author  <your name here>
 */
class RelatorioList extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        $CAPITULO_R = new TDBCombo('CAPITULO_R','matriz_referencias','Relatorio','CAPITULO_R','CAPITULO_R');
         
        $ITEM_R = new TDBCombo('ITEM_R','matriz_referencias','Relatorio','ITEM_R','ITEM_R');
        $SUBITEM_R = new TDBCombo('SUBITEM_R','matriz_referencias','Relatorio','SUBITEM_R','SUBITEM_R');
        ...
        $this->form->addActionLink(_t('New'), new TAction(['RelatorioForm', 'onEdit']), 'fa:plus green');
        ...
        $column_NR_PARAG_R = new TDataGridColumn('NR_PARAG_R', 'Nr Parag R', 'left');
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(['RelatorioForm', 'onEdit']);
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('RelatorioList_filter_ID_RELATORIO',   NULL);
        TSession::setValue('RelatorioList_filter_CAPITULO_R',   NULL);
        TSession::setValue('RelatorioList_filter_ITEM_R',   NULL);
        TSession::setValue('RelatorioList_filter_SUBITEM_R',   NULL);
        TSession::setValue('RelatorioList_filter_NR_PARAG_R',   NULL);
        TSession::setValue('RelatorioList_filter_TEXTO_R',   NULL);
        TSession::setValue('RelatorioList_filter_REF',   NULL);
        TSession::setValue('RelatorioList_filter_SUGESTAO',   NULL);

        if (isset($data->ID_RELATORIO) AND ($data->ID_RELATORIO)) {
            $filter = new TFilter('ID_RELATORIO', 'like', "%{$data->ID_RELATORIO}%"); // create the filter
            TSession::setValue('RelatorioList_filter_ID_RELATORIO',   $filter); // stores the filter in the session
        }


        if (isset($data->CAPITULO_R) AND ($data->CAPITULO_R)) {
            $filter = new TFilter('CAPITULO_R', 'like', "%{$data->CAPITULO_R}%"); // create the filter
            TSession::setValue('RelatorioList_filter_CAPITULO_R',   $filter); // stores the filter in the session
        }


        if (isset($data->ITEM_R) AND ($data->ITEM_R)) {
            $filter = new TFilter('ITEM_R', 'like', "%{$data->ITEM_R}%"); // create the filter
            TSession::setValue('RelatorioList_filter_ITEM_R',   $filter); // stores the filter in the session
        }


        if (isset($data->SUBITEM_R) AND ($data->SUBITEM_R)) {
            $filter = new TFilter('SUBITEM_R', 'like', "%{$data->SUBITEM_R}%"); // create the filter
            TSession::setValue('RelatorioList_filter_SUBITEM_R',   $filter); // stores the filter in the session
        }


        if (isset($data->NR_PARAG_R) AND ($data->NR_PARAG_R)) {
            $filter = new TFilter('NR_PARAG_R', 'like', "%{$data->NR_PARAG_R}%"); // create the filter
            TSession::setValue('RelatorioList_filter_NR_PARAG_R',   $filter); // stores the filter in the session
        }


        if (isset($data->TEXTO_R) AND ($data->TEXTO_R)) {
            $filter = new TFilter('TEXTO_R', 'like', "%{$data->TEXTO_R}%"); // create the filter
            TSession::setValue('RelatorioList_filter_TEXTO_R',   $filter); // stores the filter in the session
        }


        if (isset($data->REF) AND ($data->REF)) {
            $filter = new TFilter('REF', 'like', "%{$data->REF}%"); // create the filter
            TSession::setValue('RelatorioList_filter_REF',   $filter); // stores the filter in the session
        }


        if (isset($data->SUGESTAO) AND ($data->SUGESTAO)) {
            $filter = new TFilter('SUGESTAO', 'like', "%{$data->SUGESTAO}%"); // create the filter
            TSession::setValue('RelatorioList_filter_SUGESTAO',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue('RelatorioList_filter_ID_RELATORIO')) {
                $criteria->add(TSession::getValue('RelatorioList_filter_ID_RELATORIO')); // add the session filter
            }


            if (TSession::getValue('RelatorioList_filter_CAPITULO_R')) {
                $criteria->add(TSession::getValue('RelatorioList_filter_CAPITULO_R')); // add the session filter
            }


            if (TSession::getValue('RelatorioList_filter_ITEM_R')) {
                $criteria->add(TSession::getValue('RelatorioList_filter_ITEM_R')); // add the session filter
            }


            if (TSession::getValue('RelatorioList_filter_SUBITEM_R')) {
                $criteria->add(TSession::getValue('RelatorioList_filter_SUBITEM_R')); // add the session filter
            }


            if (TSession::getValue('RelatorioList_filter_NR_PARAG_R')) {
                $criteria->add(TSession::getValue('RelatorioList_filter_NR_PARAG_R')); // add the session filter
            }


            if (TSession::getValue('RelatorioList_filter_TEXTO_R')) {
                $criteria->add(TSession::getValue('RelatorioList_filter_TEXTO_R')); // add the session filter
            }


            if (TSession::getValue('RelatorioList_filter_REF')) {
                $criteria->add(TSession::getValue('RelatorioList_filter_REF')); // add the session filter
            }


            if (TSession::getValue('RelatorioList_filter_SUGESTAO')) {
                $criteria->add(TSession::getValue('RelatorioList_filter_SUGESTAO')); // add the session filter
            }
    ...
    public static function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('matriz_referencias'); // open a transaction with database
            $object = new Relatorio($key, FALSE); // instantiates the Active Record
